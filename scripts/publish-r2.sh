...
if ! command -v aws >/dev/null 2>&1; then
  echo "Install awscli v2 first" >&2; exit 1
fi
...
aws s3 cp "$OUT/" "s3://$R2_BUCKET/openwrt/23.05/" \
  --endpoint-url "$ENDPOINT" --recursive --acl public-read --content-type text/plain || true
...
# Optional: upload librespeed binaries if present (expect dist/librespeed/<arch>/librespeed-cli)
